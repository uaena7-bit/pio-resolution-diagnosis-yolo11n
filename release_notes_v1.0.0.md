# v1.0.0 - Final CSSP reproducibility materials

This release provides the final reproducibility materials for the CSSP submission version of the manuscript:

**Leakage-Controlled Input-Resolution Diagnosis for Dense Small-Object Detection in High-Density Visual Signals**

Included updates:

- added source data and scripts for Fig. 2, Fig. 3, and Fig. 4;
- clarified that Fig. 3 and Fig. 4 use ordinary subgroup AP, not strict global-first diagnostic AP;
- updated README and CITATION metadata for the final CSSP-oriented release;
- retained split manifests, leakage audit summaries, group definitions, bootstrap outputs, counting-calibration outputs, and evaluation scripts.

The original PIO images are not redistributed. Users should obtain the raw dataset from the original dataset source.
